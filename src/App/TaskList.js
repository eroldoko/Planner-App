...
export class TaskList {
  constructor(type) {
    ...
    this.tasks = [];
    this.type = type;
    this.removeCompleteBtnAct = document.querySelector(
      ".remove-complete-btnAct"
    );
    this.removeCompleteBtnFin = document.querySelector(
      ".remove-complete-btnFin"
    );
    this.removeCompleteBtnAct.addEventListener(
      "click",
      this.clearCompleteActive.bind(this)
    );
    this.removeCompleteBtnFin.addEventListener(
      "click",
      this.clearCompleteFinished.bind(this)
    );
    ...
    for (const taskItem of taskItems) {
      this.tasks.push(
        new TaskItem(
          taskItem.id,
          this.switchTask.bind(this, taskItem.id),
          this.type
        )
      );
    }
              ...
              el.addEventListener("click", function () {
                ...
                tasks.forEach((task) => {
                  ...
                  if (checked) {
                    ...
                    let index = finishedTasks.findIndex(
                      (item) => item.id === task.id
                    );
